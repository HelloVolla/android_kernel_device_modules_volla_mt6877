...
/*prize add by zhaopengge 20220718 ---start*/
...
/*prize add by zhaopengge 20220718 ---end*/
#else
...
static void aw_cali_disable_dsp_ng(struct aw_device *aw_dev, bool status)
{
	...
	if (status) {
		aw883xx_device_params(aw_dev, AW_DEV_DSP_NOISE_GATE_PARAMS,
			(void *)&dsp_ng_desc->disable, sizeof(unsigned int),
			AW_SET_DEV_PARAMS);
		aw883xx_device_params(aw_dev, AW_DEV_DSP_LOW_POWER_PARAMS,
			(void *)&dsp_ng_desc->dsp_lp_disable, sizeof(unsigned int),
			AW_SET_DEV_PARAMS);
	} else {
		aw883xx_device_params(aw_dev, AW_DEV_DSP_NOISE_GATE_PARAMS,
			(void *)&dsp_ng_desc->enable, sizeof(unsigned int),
			AW_SET_DEV_PARAMS);
		aw883xx_device_params(aw_dev, AW_DEV_DSP_LOW_POWER_PARAMS,
			(void *)&dsp_ng_desc->dsp_lp_enable, sizeof(unsigned int),
			AW_SET_DEV_PARAMS);
	}
}
...
static void aw_cali_svc_recover_info(struct aw_device *aw_dev)
{
	...
	if (dsp_ng_desc->reg != AW_REG_NONE) {
		aw_dev->ops.aw_reg_write_bits(aw_dev, dsp_ng_desc->reg,
			dsp_ng_desc->mask, backup_desc->dsp_ng_cfg);

		aw_dev->ops.aw_dsp_write_bits(aw_dev, dsp_ng_desc->dsp_lp_reg,
						dsp_ng_desc->dsp_lp_mask, backup_desc->dsp_lp_cfg,
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (is_single_cali) {
		len += snprintf(buf+len, PAGE_SIZE-len, "dev[%u]:%u mOhms\n",
		...
		if (ret <= 0) {
			...
			for (i = 0; i < ret; i++)
				len += snprintf(buf+len, PAGE_SIZE-len, "dev[%d]:%u mOhms ", i, re[i]);
...
static ssize_t aw_cali_class_cali_f0_q_show(const struct  class *class, const struct class_attribute *attr, char *buf)
{
	struct list_head *dev_list = NULL;
	struct aw_device *local_dev = NULL;
	int ret = -1, i = 0;
...
/* prize added for save smart PA cali_re value to NV storage start */
/*
	create /sys/fsm_drv/force_calib_fsm
	DRIVER_PATH = "/sys/fsm_drv/";
  DRV_VERSION_NODE = "/sys/fsm_drv/version";
  CALIB_R25_NODE = "/sys/fsm_drv/force_calib_fsm";
  R25_RESULT_NODE =  "/sys/fsm_drv/calib_result";
  CALIB_F0_NODE = "/sys/fsm_drv/f0_calib";
  F0_RESULT_NODE = "/sys/fsm_drv/f0_result";
*/

struct kobject *fsm_drv_kobj = NULL;
...
static ssize_t version_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	aw_dev = list_first_entry(dev_list, struct aw_device, list_node);
	if (!aw_dev) {
		aw_pr_err("get aw_dev failed");
		return ret;
	}

    if (aw_dev->ops.aw_get_version) {
			len = aw_dev->ops.aw_get_version(version_buf, sizeof(version_buf));
			if (len < 0) {
				aw_dev_err(aw_dev->dev, "get version failed");
				return -EINVAL;
			}
			count = snprintf(buf, PAGE_SIZE, "%s", version_buf);
		} else {
...
static ssize_t force_calib_fsm_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int ret, i;
	struct list_head *dev_list = NULL;
	struct aw_device *local_dev = NULL;
	...
	ret = aw_cali_svc_cali_re(local_dev, is_single_cali, CALI_OPS_HMUTE);
	if (ret < 0) {
		aw_dev_err(local_dev->dev, "cali re failed");
		return ret;
	}

	if (is_single_cali) {
		len += snprintf(buf+len, PAGE_SIZE-len, "dev[%u]:%u mOhms\n",
				local_dev->channel, local_dev->cali_desc.cali_re);
	} else {
		ret = aw_cali_svc_get_devs_cali_val(local_dev, GET_RE_TYPE, cali_re, AW_DEV_CH_MAX);
		if (ret <= 0) {
			aw_dev_err(local_dev->dev, "get re failed");
...
static ssize_t force_calib_fsm_store(struct kobject *kobj, struct kobj_attribute *attr,
	const char *buf, size_t count)
{
	...
	printk("aw883xx enter force_calib_fsm_store buf:%s", buf);
	...
	ret = kstrtoint(buf, 0, &re);
	if (ret < 0) {
		aw_dev_err(local_dev->dev, "read buf %s failed", buf);
		return ret;
	}
	printk("aw883xx force_calib_fsm_store re:%d", re);
	ret = aw_cali_store_cali_re(local_dev, re);
	if (ret < 0) {
		aw_dev_err(local_dev->dev, "error set re str %s failed", buf);
		return -EPERM;
	}
	printk("aw883xx force_calib_fsm_store ok re:%d ret:%d", local_dev->cali_desc.cali_re, ret);
	return count;
}

static ssize_t calib_result_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int ret, i;
	struct list_head *dev_list = NULL;
	struct aw_device *local_dev = NULL;
...
static ssize_t f0_calib_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int ret, i;
	struct list_head *dev_list = NULL;
	struct aw_device *local_dev = NULL;
	...
	ret = aw_cali_svc_cali_f0_q(local_dev, is_single_cali, CALI_OPS_NOISE);
	if (ret < 0) {
		aw_dev_err(local_dev->dev, "cali f0 failed");
		return ret;
	}

	if (is_single_cali) {
		len += snprintf(buf+len, PAGE_SIZE-len, "dev[%u]:%u Hz\n",
				local_dev->channel, local_dev->cali_desc.f0);
	} else {
		ret = aw_cali_svc_get_devs_cali_val(local_dev, GET_F0_TYPE, cali_f0, AW_DEV_CH_MAX);
		if (ret <= 0) {
			aw_dev_err(local_dev->dev, "get re failed");
		} else {
			for (i = 0; i < ret; i++)
				len += snprintf(buf+len, PAGE_SIZE-len, "dev[%d]:%u Hz", i, cali_f0[i]);
...
static ssize_t f0_result_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	int ret, i;
	struct list_head *dev_list = NULL;
	struct aw_device *local_dev = NULL;
...
static struct kobj_attribute dev_attr_version = __ATTR_RO(version);
static struct kobj_attribute dev_attr_force_calib_fsm = __ATTR_RW(force_calib_fsm);
static struct kobj_attribute dev_attr_calib_result = __ATTR_RO(calib_result);
static struct kobj_attribute dev_attr_f0_calib = __ATTR_RO(f0_calib);
static struct kobj_attribute dev_attr_f0_result = __ATTR_RO(f0_result);
static struct kobj_attribute dev_attr_reg = __ATTR_RW(reg);

static struct attribute *fsm_drv_attrs[] = {
	&dev_attr_version.attr,
	&dev_attr_force_calib_fsm.attr,
	&dev_attr_calib_result.attr,
	&dev_attr_f0_calib.attr,
	&dev_attr_f0_result.attr,
	&dev_attr_reg.attr,
	NULL
};

static struct attribute_group fsm_drv_attr_group = {
	.attrs = fsm_drv_attrs,
};
/* prize added for save smart PA cali_re value to NV storage end */
...
/* prize added for save smart PA cali_re value to NV storage start */
	fsm_drv_kobj = kobject_create_and_add("fsm_drv", NULL);
	aw_pr_info("aw882xx driver create fsm_drv_kobj :%p", fsm_drv_kobj);
	if (fsm_drv_kobj) {
		ret = sysfs_create_group(fsm_drv_kobj, &fsm_drv_attr_group);
		aw_pr_info("aw882xx driver create sysfs_create_file ret:%d", ret);
	}
/* prize added for save smart PA cali_re value to NV storage end */
}
