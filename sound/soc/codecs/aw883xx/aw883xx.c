...
#define AW883XX_DRIVER_VERSION "v1.6.0"
...
static int aw883xx_i2c_writes(struct aw883xx *aw883xx,
...
static int aw883xx_i2c_write(struct aw883xx *aw883xx,
...
static int aw883xx_i2c_read(struct aw883xx *aw883xx,
			uint8_t reg_addr, uint16_t *reg_data)
{
	int ret = -1;
	uint8_t cnt = 0;
...
int aw883xx_dsp_writes(struct aw883xx *aw883xx, uint8_t *data, uint32_t len, uint16_t base)
{
	int i = 0;
	struct aw_dsp_mem_desc *desc = &aw883xx->aw_pa->dsp_mem_desc;
...
#ifdef AW_DSP_I2C_WRITES
	/* i2c writes */
	aw883xx_i2c_write(aw883xx, desc->dsp_madd_reg, base);
	...
	for (i = 0; i < len; i += AW_MAX_RAM_WRITE_BYTE_SIZE) {
		...
		aw883xx_i2c_writes(aw883xx, desc->dsp_mdat_reg, &data[i], tmp_len);
	}
#else
	/* i2c write */
	aw883xx_i2c_write(aw883xx, desc->dsp_madd_reg, base);
	for (i = 0; i < len; i += 2) {
		...
		aw883xx_i2c_write(aw883xx, desc->dsp_mdat_reg, reg_val);
	}
#endif
	mutex_unlock(&aw883xx->i2c_lock);
			...
			struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (aw883xx->aw_pa->cali_desc.cali_re == AW_ERRO_CALI_RE_VALUE) {
			ret = aw883xx_device_params(aw883xx->aw_pa, AW_DEV_BIN_RE_PARAMS,
					(void *)&re, sizeof(re), AW_GET_DEV_PARAMS);
			if (ret < 0)
				aw_dev_err(aw883xx->dev, "get cali re failed");
			else
				aw883xx->aw_pa->cali_desc.cali_re = re;
...
static int aw883xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	struct aw883xx *aw883xx =
		aw_componet_codec_ops.codec_get_drvdata(codec);
	uint32_t spin_params = AW_DSP_SPIN_ST;
...
static void aw883xx_interrupt_work(struct work_struct *work)
{
	...
	/*read sysst value*/
	ret = aw883xx_device_params(aw_dev, AW_DEV_SYSST_PARAMS,
			(void *)&sys_status, sizeof(sys_status), AW_GET_DEV_PARAMS);
	if (ret < 0)
		aw_dev_err(aw883xx->dev, "get sysst value failed");
	else
		aw_dev_info(aw883xx->dev, "sysst value 0x%x", sys_status);

	ret = aw883xx_device_params(aw_dev, AW_DEV_REALTIME_CRC_GET_PARAMS,
	...
	if (crc_switch_status) {
		ret = aw883xx_crc_realtime_check(aw_dev, &crc_check_abnormal);
		if (ret < 0)
		...
		if (crc_check_abnormal) {
			...
			if (aw883xx->pstream) {
				...
				ret = aw883xx_device_fw_update(aw_dev, AW_DSP_FW_UPDATE_ON, true);
				if (ret < 0)
					aw_dev_err(aw883xx->dev, "fw update failed");
	...
	/*unmask interrupt*/
	ret = aw883xx_device_params(aw_dev, AW_DEV_INT_PARAMS,
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	ret = aw883xx_device_params(aw_dev, AW_DEV_VOLUME_PARAMS,
			(void *)&compared_vol, sizeof(compared_vol), AW_SET_DEV_PARAMS);
	if (ret < 0) {
		aw_dev_err(aw_dev->dev, "write_params failed");
		return -EINVAL;
	}
...
static int aw883xx_request_firmware_file(struct aw883xx *aw883xx)
{
	...
	ret_st = aw883xx_device_status(aw883xx->aw_pa, AW_DEV_CHECK_SPIN_MODE_STATUS,
		AW_SET_DEV_STATUS);
	if (!ret_st)
		aw_dev_err(aw883xx->dev, "set spin status failed");
...
static void aw883xx_fw_work(struct work_struct *work)
{
	struct aw883xx *aw883xx = container_of(work,
...
static int aw883xx_codec_probe(aw_snd_soc_codec_t *aw_codec)
{
	...
	INIT_DELAYED_WORK(&aw883xx->acf_work, aw883xx_fw_work);
...
static void aw883xx_codec_remove(aw_snd_soc_codec_t *aw_codec)
{
	...
	aw883xx_device_deinit(aw883xx->aw_pa);
}
#else
...
#else
static const struct snd_soc_codec_driver soc_codec_dev_aw883xx = {
	.probe = aw883xx_codec_probe,
	.remove = aw883xx_codec_remove,
};
#endif
...
static int aw883xx_componet_codec_register(struct aw883xx *aw883xx)
{
	...
	printk("pzp aw883xx_componet_codec_register ret: %d dai_drv->name: %s\n", ret, dai_drv->name);
	return 0;
}
...
static void aw883xx_parse_sync_load_dt(struct aw883xx *aw883xx)
{
	int ret = -1;
	int32_t sync_load = 0;
	struct device_node *np = aw883xx->dev->of_node;
...
static int aw883xx_parse_dt(struct aw883xx *aw883xx)
{
	...
	aw883xx_parse_sync_load_dt(aw883xx);
...
static int aw883xx_match_chipid(struct aw883xx *aw883xx)
{
	int ret = -1;
	uint16_t reg_val = 0;
	...
	ret = aw883xx_init_check_chipid(aw883xx, reg_val);
	if (ret < 0)
		return ret;
...
static irqreturn_t aw883xx_irq(int irq, void *data)
{
	...
	struct aw_device *aw_dev = aw883xx->aw_pa;
	uint32_t mask_value = AW_DEV_MASK_INT_VAL;
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (aw_dev->ops.aw_check_rd_access(aw883xx->reg_addr)) {
		aw_dev->ops.aw_reg_read(aw_dev, aw883xx->reg_addr, &reg_val);
		len += snprintf(buf + len, PAGE_SIZE - len,
				"reg:0x%02x=0x%04x\n", aw883xx->reg_addr,
				reg_val);
	}
				...
				const char *buf, size_t count)
{
	...
	struct aw_device *aw_dev = aw883xx->aw_pa;
	unsigned int databuf[2] = { 0 };
	uint32_t crc_status = AW_DSP_CRC_BYPASS;

	aw883xx_device_params(aw883xx->aw_pa, AW_DEV_CRC_FLAG_PARAMS,
				(void *)&crc_status, sizeof(crc_status), AW_SET_DEV_PARAMS);

	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		aw883xx->dsp_addr = (unsigned int)databuf[0];
		aw_dev->ops.aw_dsp_write(aw_dev, databuf[0], databuf[1], AW_DSP_16_DATA);
...
static int aw883xx_awrw_write(struct aw883xx *aw883xx, const char *buf, size_t count)
{
	...
	for (i = 0; i < data_len; i++) {
		ret = sscanf(buf + AWRW_HDR_LEN + 1 + i * 5, "0x%02x", &temp_data);
		if (ret <= 0) {
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (kstrtoint(buf, 10, &databuf) == 0) {
		if (databuf > (aw883xx->aw_pa->volume_desc.mute_volume)) {
			aw_dev_info(aw883xx->dev, "step overflow %d", databuf);
			return count;
		}

		ret = aw883xx_device_params(aw_dev, AW_DEV_FADE_STEP_PARAMS,
				(void *)&databuf, sizeof(databuf), AW_SET_DEV_PARAMS);
		if (ret < 0) {
			aw_dev_err(aw_dev->dev, "write_params failed");
			return ret;
		}
	}
	aw_dev_info(aw883xx->dev, "set step %d Done", databuf);
	...
	struct device_attribute *attr, char *buf)
{
	...
	ret = aw883xx_device_params(aw_dev, AW_DEV_FADE_STEP_PARAMS,
			(void *)&step, sizeof(step), AW_GET_DEV_PARAMS);
	if (ret < 0) {
		aw_dev_err(aw_dev->dev, "read_params failed");
		return ret;
	}
				...
				struct device_attribute *attr, char *buf)
{
	...
	ret = aw883xx_device_params(aw_dev, AW_DEV_TE_PARAMS,
			(void *)&te, sizeof(te), AW_GET_DEV_PARAMS);
	if (ret < 0)
		return ret;
	...
	struct device_attribute *attr, char *buf)
{
	...
	ret = aw883xx_device_params(aw883xx->aw_pa, AW_DEV_CALI_RE_PARAMS,
		(void *)&re, sizeof(re), AW_GET_DEV_PARAMS);
	if (ret < 0) {
		aw_dev_err(aw883xx->dev, "read dsp re fail");
		return ret;
	}
	...
	if (aw883xx->aw_pa->dsp_cfg == AW_DEV_DSP_BYPASS) {
		...
		if (!status) {
			len += snprintf((char *)(buf + len),
					...
					__func__);
			aw_dev_err(aw883xx->dev, "PA status abnormal, dsp show failed");
			return len;
		}
		...
		data_len = (uint32_t)(2 * (uint32_t)(dsp_st_desc->dsp_reg_e1 - dsp_st_desc->dsp_reg_s1));
		ret = aw883xx_dsp_log_info(aw883xx, dsp_st_desc->dsp_reg_s1,
			data_len, "dsp_st");
		if (ret < 0) {
			aw_dev_err(aw883xx->dev, "dsp_config:0x%04x-0x%04x failed",
				dsp_st_desc->dsp_reg_s1, dsp_st_desc->dsp_reg_e1);
			return len;
		}
		...
		data_len = (uint32_t)(2 * (uint32_t)(dsp_st_desc->dsp_reg_e2 - dsp_st_desc->dsp_reg_s2));
 ...
 *
 ******************************************************/
 #if defined AW_KERNEL_VER_OVER_6_6_0
...
#endif
{
	...
	/* hardware reset */
	ret = aw883xx_hw_reset(aw883xx);
	if (ret)
		return ret;

	/* aw883xx chip id */
	ret = aw883xx_match_chipid(aw883xx);
	if (ret < 0) {
		aw_dev_err(&i2c->dev, "aw883xx_match_chipid failed ret=%d", ret);
		return ret;
	}
