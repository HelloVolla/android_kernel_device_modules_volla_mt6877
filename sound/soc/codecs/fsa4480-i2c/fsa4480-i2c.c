	...
	struct device *dev)
{
    ...
    if ((20630 == get_project()) || (20631 == get_project()) || (20632 == get_project())) {
        ...
        if (IS_ERR(pinctrl1)) {
            ...
            pr_err("can not find fsa4480 pinctrl 0\n");
            return ret;
        }
...
static int fsa4480_probe(struct i2c_client *i2c)
{
	...
	if (tcpm_inquire_typec_attach_state(fsa_priv->tcpc_dev) == TYPEC_ATTACHED_AUDIO) {
		dev_info(fsa_priv->dev, "%s: Audio accessory attached during initial state\n", __func__);
		fsa_priv->plug_state = true;
		schedule_work(&fsa_priv->usbc_analog_work);
	}

	fsa_priv->pd_nb.notifier_call = fsa4480_usbc_event_changed;
	fsa_priv->pd_nb.priority = 10;//drv Solve the problem that the headset is not recognized-pengzhipeng-20231107
	rc = register_tcp_dev_notifier(fsa_priv->tcpc_dev, &fsa_priv->pd_nb, TCP_NOTIFY_TYPEC_STATE);//drv Solve the problem that the headset is not recognized-pengzhipeng-20231107
	if (rc < 0) {
