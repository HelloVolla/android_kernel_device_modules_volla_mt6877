...
static void accdet_get_efuse(void)
{
	 ...
	 */
	ret = nvmem_device_read(accdet->accdet_efuse, 109*2, 2, &efuseval);
 ...
 */
	...
	ret = nvmem_device_read(accdet->accdet_efuse, 112*2, 2, &efuseval);
...
static void accdet_get_efuse_4key(void)
{
	 ...
	 */
	ret = nvmem_device_read(accdet->accdet_efuse, 110*2, 2, &tmp_val);
...
static void eint_work_callback(struct work_struct *work)
{
	if (accdet->cur_eint_state == EINT_PLUG_IN) {
		...
		accdet_init();
		printk("pzp eint_work_callback EINT_PLUG_IN\n");	
		enable_accdet(0);
	} else {
		mutex_lock(&accdet->res_lock);
		...
		accdet_clear_bit(ACCDET_SW_EN_ADDR,
			ACCDET_SW_EN_SFT);
		disable_accdet();
		printk("pzp eint_work_callback EINT_PLUG_OUT\n");
		headset_plug_out();
	}
...
static void config_eint_init_by_mode(void)
{
	...
	if ((accdet_dts.eint_detect_mode == 0x1) ||
		...
		pr_info("%s: %x=%x %x=%x",
			...
			accdet_read(RG_EINT0CONFIGACCDET_ADDR),
			RG_EINTCOMPVTH_ADDR, accdet_read(RG_EINTCOMPVTH_ADDR));
...
//drv Solve the problem that the headset is not recognized-pengzhipeng-20231107-end
	...
	if (tcpm_inquire_typec_attach_state(accdet->tcpc_dev) == TYPEC_ATTACHED_AUDIO) {
		pr_info("%s: Audio accessory attached during initial state\n", __func__);
		accdet_eint_func_extern(EINT_PLUG_IN);
	}
