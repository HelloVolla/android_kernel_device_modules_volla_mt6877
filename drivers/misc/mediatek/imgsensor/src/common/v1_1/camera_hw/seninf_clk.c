					...
					struct ACDK_SENSOR_MCLK_STRUCT *pmclk)
{
	...
	if (pmclk->on) {
		...
		if(pmclk->TG == 1){// main sensor TG
			PK_PR_ERR("[CAMERA SENSOR] CCF kdSetSensorMclk for TG 4 on\n");
			if (clk_prepare_enable(pclk->clk_sel[idx_tg+3]))// TG 2 for dual camera
			...
			// else
				// atomic_inc(&pclk->enable_cnt[idx_freq]);
			ret = clk_set_parent(pclk->clk_sel[idx_tg+3], pclk->clk_sel[idx_freq]);
		}
		#endif
		...
		#if IS_ENABLED(CONFIG_PRIZE_DUAL_CAMERA_ENABLE)
		if(pmclk->TG == 1){
			PK_PR_ERR("[CAMERA SENSOR] CCF kdSetSensorMclk for TG 4 off\n");
			if (atomic_read(&pclk->enable_cnt[idx_tg+3]) > 0) {
				clk_disable_unprepare(pclk->clk_sel[idx_tg+3]);
