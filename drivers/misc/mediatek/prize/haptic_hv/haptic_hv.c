...
#endif
//prize add by wangfei for hardware info 20221027 end

//prize add for haptic cali start
			      ...
			      const char *buf, size_t count)
{
	...
	if (val <= 0)
		return count;
	printk("aw_haptic duration=%d", val);
...
static const char TAG_NAME[] = "androidboot.aw_cali_data=";

static int haptic_get_cmdline_cali_data(void)
{
...
/*static int haptic_get_cmdline_cali_data(void)
{
	char *ptr = NULL;
	int cali_data = 0;
	ptr = strstr(saved_command_line,"androidboot.aw_cali_data=");
	if(ptr == NULL)
	{
...
static int aw_i2c_probe(struct i2c_client *i2c)
{
	...
	//prize add by wangfei for hardware info 20221027 start 
	#ifdef CONFIG_PRIZE_HARDWARE_INFO
	sprintf(current_line_motor_info.id,"0x%x", aw_haptic->chipid);
	strcpy(current_line_motor_info.vendor, "awinic");
	strcpy(current_line_motor_info.chip, "AW86927");
	#endif
	//prize add by wangfei for hardware info 20221027 end
...
static int __init aw_i2c_init(void)
{
	...
	printk("[wolf]<%s>%s: aw_haptic driver version %s\n", AW_I2C_NAME, __func__,
