...
#define CONFIG_NFC_SHUTDOWN_WORKAROUND //pri modify for nfc shutdown hce
...
#endif
#ifdef CONFIG_SEC_NFC_WAKE_LOCK
    info->nfc_wake_lock.dev = dev;
    wake_lock_init(&info->nfc_wake_lock, WAKE_LOCK_SUSPEND, "nfc_wake_lock");
#endif

#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
    strcpy(current_nfc_info.chip,   "S3NRN4VX");
    sprintf(current_nfc_info.id,    "0x%04x", 0x27);
    strcpy(current_nfc_info.vendor, "Samsung");
