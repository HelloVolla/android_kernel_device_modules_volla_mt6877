...
/* prize added for nfc hardware info 20221026 start */
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
	strcpy(current_nfc_info.chip,   "ST21NFCD");
	sprintf(current_nfc_info.id,    "0x%02x", client->addr);
	strcpy(current_nfc_info.vendor, "ST");
	strcpy(current_nfc_info.more,   "nfc");
#endif
/* prize added for nfc hardware info 20221026 end */
