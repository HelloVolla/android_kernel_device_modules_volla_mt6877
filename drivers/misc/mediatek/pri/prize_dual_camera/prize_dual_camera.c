...
#define GC2375H_SERIAL_ADDR	0x17
#define BF2257CS_SERIAL_ADDR 0x3e
#define BF30A2_SERIAL_ADDR   0x6e
...
static DEFINE_MUTEX(dcam_r1_mutex);

static struct i2c_client *g_dcam_r_client = NULL;
static struct i2c_client *g_dcam_r1_client = NULL;
...
int kdCISModulePowerOn(struct i2c_client* client,  bool On)
{
	...
	{
	//GC02M1B
		printk("GC02M1B set power start\n");
		if (On){
			////set_avdd_regulator(1);
	...
	else if(client->addr == BF2257CS_SERIAL_ADDR) 
	{
		if (On){
			//gpio_direction_output(dcam_data->rst_pin,0);
			//mdelay(5);
	...
	else if(client->addr == BF30A2_SERIAL_ADDR) 
	{
		if (On){
			//set_avdd_regulator(1);
			mdelay(5);
...
static void CameraStreamOn(struct i2c_client *client)
{
    ...
    else if(sensor_id == BF2257CS_SENSOR_ID)
		client->addr = BF2257CS_SERIAL_ADDR;
    else if(sensor_id == BF30A2_SENSOR_ID)
		client->addr = BF30A2_SERIAL_ADDR;		
	else
		client->addr = 0x3c;
...
static void GC2375H_Sensor_Init(struct i2c_client *client)
{
	...
	write_cmos_sensor(client,0x2b,0x08); 
	write_cmos_sensor(client,0xfe,0x00);            
	write_cmos_sensor(client,0x8f,read_cmos_sensor(client, 0x8f) | 0x17);
};
...
void BF2257CS_Sensor_Init(struct i2c_client *client)
{
	write_cmos_sensor(client,0xf2, 0x01);
    write_cmos_sensor(client,0x00, 0x01);//Bit[3]: Mirror Bit[2]: vFlip
	write_cmos_sensor(client,0x02, 0xb7);
	write_cmos_sensor(client,0x1e, 0x04);
	write_cmos_sensor(client,0x24, 0x60);
	write_cmos_sensor(client,0xe0, 0x00);
	write_cmos_sensor(client,0xe1, 0x01);
	write_cmos_sensor(client,0xe2, 0x08);
	write_cmos_sensor(client,0xe5, 0xe3);
	write_cmos_sensor(client,0xe6, 0x60);
	write_cmos_sensor(client,0xe7, 0x33);
	write_cmos_sensor(client,0xe8, 0x12);
	write_cmos_sensor(client,0xe9, 0x89);
	write_cmos_sensor(client,0xea, 0x87);
	write_cmos_sensor(client,0xeb, 0x80);
	write_cmos_sensor(client,0xec, 0x91);
	write_cmos_sensor(client,0xed, 0x60);

	//MIPICLK:672M
	write_cmos_sensor(client,0xe3, 0x78);
	write_cmos_sensor(client,0xe4, 0xe0);

	//Dummy
	write_cmos_sensor(client,0x06, 0x10);
	write_cmos_sensor(client,0x07, 0x00);
	write_cmos_sensor(client,0x0b, 0x80);
	write_cmos_sensor(client,0x0c, 0x03);

	//Black target
	write_cmos_sensor(client,0x59, 0x40);
	write_cmos_sensor(client,0x5a, 0x40);
	write_cmos_sensor(client,0x5b, 0x40);
	write_cmos_sensor(client,0x5c, 0x40);

	//MIPI Setting
	write_cmos_sensor(client,0x70, 0x08);
	write_cmos_sensor(client,0x71, 0x07);
	write_cmos_sensor(client,0x72, 0x12);
	write_cmos_sensor(client,0x73, 0x09);
	write_cmos_sensor(client,0x74, 0x08);
	write_cmos_sensor(client,0x75, 0x06);
	write_cmos_sensor(client,0x76, 0x20);
	write_cmos_sensor(client,0x77, 0x02);
	write_cmos_sensor(client,0x78, 0x10);
	write_cmos_sensor(client,0x79, 0x09);
	write_cmos_sensor(client,0x7a, 0x00);
	write_cmos_sensor(client,0x7b, 0x00);
	write_cmos_sensor(client,0x7c, 0x00);
	write_cmos_sensor(client,0x7d, 0x0f);

	//Window 1600*1200
	write_cmos_sensor(client,0xca, 0x60);
	write_cmos_sensor(client,0xcb, 0x40);
	write_cmos_sensor(client,0xcc, 0x04);
	write_cmos_sensor(client,0xcd, 0x44);
	write_cmos_sensor(client,0xce, 0x04);
	write_cmos_sensor(client,0xcf, 0xb4);

	write_cmos_sensor(client,0x6a, 0x0f);//Global gain
	write_cmos_sensor(client,0x6b, 0x04);
	write_cmos_sensor(client,0x6c, 0xe1);//{0x6b,0x6c}:int_time
	write_cmos_sensor(client,0x6d, 0x0f);

	write_cmos_sensor(client,0x01, 0x4b);//stream_on
	write_cmos_sensor(client,0xf3, 0x00);
	write_cmos_sensor(client,0xd0, 0x00);
	write_cmos_sensor(client,0x01, 0x43);
}
void BF30A2_Sensor_Init(struct i2c_client *client)
{
	write_cmos_sensor(client, 0xf2, 0x01);
	write_cmos_sensor(client, 0x12, 0x20);
	write_cmos_sensor(client, 0x15, 0x80);
	write_cmos_sensor(client, 0x6b, 0x71);
	write_cmos_sensor(client, 0x04, 0x00);
	write_cmos_sensor(client, 0x06, 0x26);
	write_cmos_sensor(client, 0x08, 0x07);
	write_cmos_sensor(client, 0x1c, 0x12);
	write_cmos_sensor(client, 0x1e, 0x26);
	write_cmos_sensor(client, 0x1f, 0x01);
	write_cmos_sensor(client, 0x20, 0x20);
	write_cmos_sensor(client, 0x21, 0x20);
	write_cmos_sensor(client, 0x34, 0x00);
	write_cmos_sensor(client, 0x35, 0x00);
	write_cmos_sensor(client, 0x36, 0x21);
	write_cmos_sensor(client, 0x37, 0x13);
	write_cmos_sensor(client, 0xca, 0x03);
	write_cmos_sensor(client, 0xcb, 0x22);
	write_cmos_sensor(client, 0xcc, 0x89);
	write_cmos_sensor(client, 0xcd, 0x6c);
	write_cmos_sensor(client, 0xce, 0x6b);
	write_cmos_sensor(client, 0xcf, 0xf0);
	write_cmos_sensor(client, 0xa0, 0x8e);
	write_cmos_sensor(client, 0x01, 0x1b);
	write_cmos_sensor(client, 0x02, 0x1d);
	write_cmos_sensor(client, 0x13, 0x48);
	write_cmos_sensor(client, 0x87, 0x20);
	write_cmos_sensor(client, 0x8a, 0x33);
	write_cmos_sensor(client, 0x8b, 0x70);
	write_cmos_sensor(client, 0x70, 0x1f);
	write_cmos_sensor(client, 0x71, 0x40);
	write_cmos_sensor(client, 0x72, 0x0a);
	write_cmos_sensor(client, 0x73, 0x62);
	write_cmos_sensor(client, 0x74, 0xa2);
	write_cmos_sensor(client, 0x75, 0xbf);
	write_cmos_sensor(client, 0x76, 0x02);
	write_cmos_sensor(client, 0x77, 0xcc);
	write_cmos_sensor(client, 0x40, 0x32);
	write_cmos_sensor(client, 0x41, 0x28);
	write_cmos_sensor(client, 0x42, 0x26);
	write_cmos_sensor(client, 0x43, 0x1d);
	write_cmos_sensor(client, 0x44, 0x1a);
	write_cmos_sensor(client, 0x45, 0x14);
	write_cmos_sensor(client, 0x46, 0x11);
	write_cmos_sensor(client, 0x47, 0x0f);
	write_cmos_sensor(client, 0x48, 0x0e);
	write_cmos_sensor(client, 0x49, 0x0d);
	write_cmos_sensor(client, 0x4b, 0x0c);
	write_cmos_sensor(client, 0x4c, 0x0b);
	write_cmos_sensor(client, 0x4e, 0x0a);
	write_cmos_sensor(client, 0x4f, 0x09);
	write_cmos_sensor(client, 0x50, 0x09);
	write_cmos_sensor(client, 0x24, 0x50);
	write_cmos_sensor(client, 0x25, 0x36);
	write_cmos_sensor(client, 0x80, 0x00);
	write_cmos_sensor(client, 0x81, 0x20);
	write_cmos_sensor(client, 0x82, 0x40);
	write_cmos_sensor(client, 0x83, 0x30);
	write_cmos_sensor(client, 0x84, 0x50);
	write_cmos_sensor(client, 0x85, 0x30);
	write_cmos_sensor(client, 0x86, 0xd8);
	write_cmos_sensor(client, 0x89, 0x45);
	write_cmos_sensor(client, 0x8f, 0x81);
	write_cmos_sensor(client, 0x91, 0xff);
	write_cmos_sensor(client, 0x92, 0x08);
	write_cmos_sensor(client, 0x94, 0x82);
	write_cmos_sensor(client, 0x95, 0xfd);
	write_cmos_sensor(client, 0x9a, 0x20);
	write_cmos_sensor(client, 0x9e, 0xbc);
	write_cmos_sensor(client, 0xf0, 0x8f);
	write_cmos_sensor(client, 0xf1, 0x02);
	write_cmos_sensor(client, 0x51, 0x06);
	write_cmos_sensor(client, 0x52, 0x25);
	write_cmos_sensor(client, 0x53, 0x2b);
	write_cmos_sensor(client, 0x54, 0x0f);
	write_cmos_sensor(client, 0x57, 0x2a);
	write_cmos_sensor(client, 0x58, 0x22);
	write_cmos_sensor(client, 0x59, 0x2c);
	write_cmos_sensor(client, 0x23, 0x33);
	write_cmos_sensor(client, 0xa0, 0x8f);
	write_cmos_sensor(client, 0xa1, 0x13);
	write_cmos_sensor(client, 0xa2, 0x0f);
	write_cmos_sensor(client, 0xa3, 0x2a);
	write_cmos_sensor(client, 0xa4, 0x08);
	write_cmos_sensor(client, 0xa5, 0x26);
	write_cmos_sensor(client, 0xa7, 0x80);
	write_cmos_sensor(client, 0xa8, 0x80);
	write_cmos_sensor(client, 0xa9, 0x1e);
	write_cmos_sensor(client, 0xaa, 0x19);
	write_cmos_sensor(client, 0xab, 0x18);
	write_cmos_sensor(client, 0xae, 0x50);
	write_cmos_sensor(client, 0xaf, 0x04);
	write_cmos_sensor(client, 0xc8, 0x10);
	write_cmos_sensor(client, 0xc9, 0x15);
	write_cmos_sensor(client, 0xd3, 0x0c);
	write_cmos_sensor(client, 0xd4, 0x16);
	write_cmos_sensor(client, 0xee, 0x06);
	write_cmos_sensor(client, 0xef, 0x04);
	write_cmos_sensor(client, 0x55, 0x30);
	write_cmos_sensor(client, 0x56, 0x9c);
	write_cmos_sensor(client, 0xb1, 0x98);
	write_cmos_sensor(client, 0xb2, 0x98);
	write_cmos_sensor(client, 0xb3, 0xc4);
	write_cmos_sensor(client, 0xb4, 0x0c);
	write_cmos_sensor(client, 0x00, 0x40);
	write_cmos_sensor(client, 0x13, 0x48);
}
/*************************************************************************
 * FUNCTION
...
static UINT32 CameraOpen(struct i2c_client *client)
{
	...
	if(!id_status)
	{
		for(i=0;i<3;i++)
		{
			client->addr = BF2257CS_SERIAL_ADDR;
			kdCISModulePowerOn(client,true);
			sensor_id=(read_cmos_sensor(client,0xfc) << 8) |(read_cmos_sensor(client,0xfd));
			//sensor_id=read_cmos_sensor(client,0xf0);
			CAMERA_DBG("BF2257CS *sensorID=%x %s %d\n",sensor_id,__func__,__LINE__);
			if(sensor_id != BF2257CS_SENSOR_ID)  
			{
				CAMERA_DBG("BF2257CS mipi Read Sensor ID Fail[open] = 0x%x\n", sensor_id); 
				//return ERROR_SENSOR_CONNECT_FAIL;
			}else{
				g_dcam_r_client->addr = BF2257CS_SERIAL_ADDR;
				id_status = 1;
				break;
			}
		}
	...
	if(!id_status)
	{
		for(i=0;i<3;i++)
		{
			...
			kdCISModulePowerOn(client,true);
			sensor_id=(read_cmos_sensor(client,0xf0) << 8) |(read_cmos_sensor(client,0xf1));
			CAMERA_DBG("GC2375H *sensorID=%x %s %d\n",sensor_id,__func__,__LINE__);
	...
	if(!id_status)
	{
		for(i=0;i<3;i++)
		{
			client->addr = BF30A2_SERIAL_ADDR;
			kdCISModulePowerOn(client,true);
			sensor_id = read_cmos_sensor(client,0xfc);
			//sensor_id=read_cmos_sensor(client,0xf0);
			CAMERA_DBG("BF30A2 *sensorID=%x %s %d\n",sensor_id,__func__,__LINE__);
			if(sensor_id != BF30A2_SENSOR_ID)  
			{
				CAMERA_DBG("BF30A2 mipi Read Sensor ID Fail[open] = 0x%x\n", sensor_id); 
				//return ERROR_SENSOR_CONNECT_FAIL;
			}else{
				g_dcam_r_client->addr = BF30A2_SERIAL_ADDR;
				id_status = 1;
				break;
			}
		}
...
static kal_uint16 Camera_Read_Shutter(struct i2c_client *client)
{
	...
	if(sensor_id == GC6133_SENSOR_ID)
		...
		client->addr = GC6153_SERIAL_ADDR;
	else if(sensor_id == GC02M1B_SENSOR_ID)//prize yanrenjie added 6153 20190416
		client->addr = GC02M1B_SERIAL_ADDR;	
    else if(sensor_id == BF2257CS_SENSOR_ID)
		client->addr = BF2257CS_SERIAL_ADDR;	
    else if(sensor_id == BF30A2_SENSOR_ID)
	...
	{
		printk("client->addr = GC2375H_SERIAL_ADDR;");
		client->addr = GC2375H_SERIAL_ADDR;
	}
	else
		client->addr = 0x3c;
	if(sensor_id == BF2257CS_SENSOR_ID){
		temp_reg1 = read_cmos_sensor(client, 0x64);
		temp_reg2 = 0;
		...
		if(temp_reg1 > 216)
			temp_reg1 = 216;
		shutter = 2000-(temp_reg1-16) * 10;//0~2000
		CAMERA_DBG("camera_Read_Shutter %d\r\n",shutter);
	    return shutter;
	}
	if(sensor_id == BF30A2_SENSOR_ID){
		temp_reg1 = read_cmos_sensor(client, 0x88);
		temp_reg2 = 0;
		/* 15- 255 15 dark 255 brigth */
		if(temp_reg1 < 16)
			temp_reg1 = 16;
	...
	if(sensor_id == GC2375H_SENSOR_ID) {
		shutter = read_cmos_sensor(client,0x3B);
		CAMERA_DBG("GC2375H camera_Read_Shutter real %d \n",shutter);
		if(shutter > 100)
			shutter =99;
		shutter = 2000 - shutter*20;
		CAMERA_DBG("GC2375H  shutter %d \n",shutter);
		return shutter;
	}
...
//--------------------------------------------------------------------------
static ssize_t show_camera_rear1_shutter_value(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct i2c_client *client = g_dcam_r1_client;
	struct dcam_data_t *dcam_data = i2c_get_clientdata(client);
	int shutter_value = 0;
...
static ssize_t store_camera_rear1_shutter_value(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	return count;
}
static struct kobj_attribute camera_rear1_value_attr = {
	.attr = {
		 .name = "dcam_r1_value",
		 .mode = 0644,
		 },
	.show = show_camera_rear1_shutter_value,
	.store = store_camera_rear1_shutter_value,
};

static ssize_t show_camera_rear1_open(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct i2c_client *client = g_dcam_r1_client;
	struct dcam_data_t *dcam_data = i2c_get_clientdata(client);
	int ret = -1;
...
static ssize_t store_camera_rear1_open(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	return count;
}
static struct kobj_attribute camera_rear1_open_attr = {
	.attr = {
		 .name = "dcam_r1_open",
		 .mode = 0644,
		 },
	.show = show_camera_rear1_open,
	.store = store_camera_rear1_open,
};
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
static ssize_t show_camera_rear1_close(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct i2c_client *client = g_dcam_r1_client;
	struct dcam_data_t *dcam_data = i2c_get_clientdata(client);
...
static ssize_t store_camera_rear1_close(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	return count;
}
static struct kobj_attribute camera_rear1_close_attr = {
	.attr = {
		 .name = "dcam_r1_close",
		 .mode = 0644,
		 },
	.show = show_camera_rear1_close,
	.store = store_camera_rear1_close,
};
//----------------------------------------------------------------------------
...
static const struct of_device_id __maybe_unused dcam_of_match[] = {
	...
	{ .compatible = "prize,dcam_r1", .data = &dcam_r1_devtype },
...
static const struct i2c_device_id dcam_id_table[] = {
	...
	{ "DCAM_R1",	(kernel_ulong_t)&dcam_r1_devtype, },
	{ }
};
MODULE_DEVICE_TABLE(i2c, dcam_id_table);

static int dcam_i2c_probe(struct i2c_client *client){
	int ret;
	...
	if (!strcmp(client->name,"dcam_r")){
		...
		gpio_direction_output(dcam_data->pdn_pin,0);
        //prize-chj-20190522-Close the back frame after booting according to the timing requirements start
		gpio_direction_output(dcam_data->pdn_pin,1);
	...
	}	else if (!strcmp(client->name,"dcam_r1")){
		g_dcam_r1_client = client;
		node = of_find_compatible_node(NULL,NULL,"prize,dcam_r1");
		dcam_data->pdn_pin = of_get_named_gpio(node,"pdn_pin",0);
		dcam_data->rst_pin = of_get_named_gpio(node,"rst_pin",0);

		printk("DCAM dcam_r1_pdn_pin(%d)  dcam_data->pdn_pin(%d)\n",dcam_data->pdn_pin,dcam_data->pdn_pin);
		gpio_request(dcam_data->pdn_pin,"dcam_r1");
		gpio_direction_output(dcam_data->pdn_pin,0);
        //prize-chj-20190522-Close the back frame after booting according to the timing requirements start
		gpio_direction_output(dcam_data->pdn_pin,1);
        //prize-chj-20190522-Close the back frame after booting according to the timing requirements end
		gpio_request(dcam_data->rst_pin,"dcam_r1");
		gpio_direction_output(dcam_data->rst_pin,0);
		ret = of_property_read_u32(node,"sensor_type",&dcam_data->sensor_type);
		if (ret){
			printk("DCAM get sensor_type fail ret(%d)\n",ret);
		...
		if (NULL != g_dcam_r1_client){
			ret =  sysfs_create_file(dcam_kobj, &camera_rear1_value_attr.attr);
			if (ret)
				CAMERA_DBG(" kernel sysfs_create_file error \r\n");
			ret =  sysfs_create_file(dcam_kobj, &camera_rear1_open_attr.attr);
			if (ret)
				CAMERA_DBG(" kernel sysfs_create_file error \r\n");
			ret =  sysfs_create_file(dcam_kobj, &camera_rear1_close_attr.attr);
			if (ret)
				CAMERA_DBG(" kernel sysfs_create_file error \r\n");
		}
...
static void  dcam_i2c_remove(struct i2c_client *client)
{
	struct dcam_data_t *dcam_data = i2c_get_clientdata(client);
...
static void __exit dcam_exit(void){
	...
	if (NULL != g_dcam_r1_client){	   
		sysfs_remove_file(dcam_kobj, &camera_rear1_value_attr.attr);
		sysfs_remove_file(dcam_kobj, &camera_rear1_open_attr.attr);
		sysfs_remove_file(dcam_kobj, &camera_rear1_close_attr.attr);
	}	
	if (dcam_kobj){
