...
#endif
		//*val = get_battery_current()*1000;

		if(!chip->cw_bat){
			chip->cw_bat = power_supply_get_by_name("cw-bat");
			if(!chip->cw_bat) {
				CP_ERR("gezi failed to get cw_bat too!\n");
			}
			else {
				ret = power_supply_get_property(chip->cw_bat,POWER_SUPPLY_PROP_CURRENT_NOW, &prop);
				*val = prop.intval;
				goto out;
			}
		} else {
...
*/
static void SC8551_i2c_remove(struct i2c_client *client)
{
	struct SC8551_chip *chip = i2c_get_clientdata(client);
