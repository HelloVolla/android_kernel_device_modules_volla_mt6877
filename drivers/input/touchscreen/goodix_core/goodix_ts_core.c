...
struct goodix_ts_core *g_ts_core = NULL;
...
static int rawdata_proc_open(struct inode *inode, struct file *file)
{
	return single_open_size(file, rawdata_proc_show,
			pde_data(inode), PAGE_SIZE * 10); // 修改为 pde_data
}
...
//prize add by xuejian, for add set report mode 20220217 start 
static RAW_NOTIFIER_HEAD(report_mode_chain);
int register_report_mode_notifier(struct notifier_block *nb)
{
  return raw_notifier_chain_register(&report_mode_chain, nb);
}

int unregister_report_mode_notifier(struct notifier_block *nb)
{
  return raw_notifier_chain_unregister(&report_mode_chain, nb);
}

int tp_mode_notifier_call_chain(unsigned long val, void *v)
{
  return raw_notifier_call_chain(&report_mode_chain, val, v);
}
EXPORT_SYMBOL(tp_mode_notifier_call_chain);


static int set_report_mode_event(struct notifier_block *self, unsigned long event, void *ptr)
{
	unsigned short val = (unsigned short)event;
	unsigned char *str = (unsigned char *)ptr;
	struct goodix_ts_core *cd = container_of(self,
			struct goodix_ts_core, report_mode_notifier);
	const struct goodix_ts_hw_ops *hw_ops = cd->hw_ops;	
	
	ts_info("%s val:%d str:%s\n", __func__, val, str);
	atomic_set(&cd->chg_state, val);

	if (!strncmp(str, "charging", 8)) {
		hw_ops->receive_cmd(cd, 0xAF, val);
	} else if (!strncmp(str, "game", 4)) {
		hw_ops->receive_cmd(cd, 0xC2, val);
	}
	return 0;
}

//prize add by xuejian, for add set report mode 20220217 end
 ...
 */
#if 0
static struct regulator *vdd_vtp;
extern struct i2c_client *global_client;

int prize_vtp_power_init(struct i2c_client *client)
{
	int ret;
	vdd_vtp = regulator_get(&client->dev, "vtouch");
	if (IS_ERR_OR_NULL(vdd_vtp)) {
		ts_info("regulator get of vdd_vtp failed");
		ret = PTR_ERR(vdd_vtp);
		vdd_vtp = NULL;
		return ret;
	}
	
	if (regulator_count_voltages(vdd_vtp) > 0) 
	{
        ret = regulator_set_voltage(vdd_vtp,2800000, 2800000);
        if (ret) {
            ts_info("vdd regulator set_vtg failed ret=%d", ret);
            regulator_put(vdd_vtp);
			return ret;
        }
		
		ret = regulator_enable(vdd_vtp);
    }
...
#endif
#if 0 
int vtp_power_switch(struct i2c_client *client, int on)
{
	int ret;
	if (IS_ERR_OR_NULL(vdd_vtp)) {
	    ts_info("regulator get of vdd_ana failed");
	   return -1;
	}
	if (on) {
		ts_info("GTP power on.");
		ret = regulator_enable(vdd_vtp);
		ts_info("GTP power on.ret=%d",ret);
	} else {
		ts_info("GTP power off.");
		ret = regulator_disable(vdd_vtp);
	}
	return ret;
} 
#endif
...
//	prize_vtp_power_init(global_client);
...
static int goodix_ts_power_init(struct goodix_ts_core *core_data)
{
	...
	if (strlen(ts_bdata->avdd_name)) {
		...
		if(ret) {
			ts_err("Failed to regulator_set_voltage\n");
		}
		ts_info("[goodix_ts_power_init] avdd is ok avdd = %d\n",regulator_get_voltage(core_data->avdd));
...
static int goodix_ts_disp_notifier_callback(struct notifier_block *nb, unsigned long event, void *v)
{
	int *data = (int *)v;
	...
	if (!ts || !data) {
		ts_err("null data or ts in %s", __func__);
		return -EINVAL;
	}
	switch (event) {
		case MTK_DISP_EARLY_EVENT_BLANK:
			if (*data == MTK_DISP_BLANK_POWERDOWN || *data == MTK_DISP_BLANK_DOZE_ENABLE) {
				goodix_ts_suspend(ts);
			}
			break;
		case MTK_DISP_EVENT_BLANK:
			if (*data == MTK_DISP_BLANK_UNBLANK || *data == MTK_DISP_BLANK_DOZE_DISABLE) {
				goodix_ts_resume(ts);
			}
			break;
	}
...
int goodix_ts_stage2_init(struct goodix_ts_core *cd)
{
	...
	ret = mtk_disp_notifier_register("Touch_driver", &cd->disp_notifier);
	if(ret) {
		ts_info("register mtk_disp_notifier_register failed. ret=%d\n", ret);
	}
#endif
	/* create sysfs files */
...
/*prize add by xuejian for gesture end*/
	...
	core_data->report_mode_notifier.notifier_call = set_report_mode_event;
	register_report_mode_notifier(&core_data->report_mode_notifier);
	/* debug node init */
	goodix_tools_init();
...
static int goodix_ts_remove(struct platform_device *pdev)
{
	...
	unregister_report_mode_notifier(&core_data->report_mode_notifier);
	goodix_tools_exit();
...
#if 0//IS_ENABLED(CONFIG_TOUCHSCREEN_GOODIX_BRL_SPI)
