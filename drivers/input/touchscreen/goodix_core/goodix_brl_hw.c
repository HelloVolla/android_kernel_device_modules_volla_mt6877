...
int brl_gesture(struct goodix_ts_core *cd, int gesture_type)
{
	...
	ts_info("brl_gesture  end");
	return 0;
}

#define GOODIX_GAME_CMD		0xC2
#define GOODIX_CHARGING_CMD		0xAF
#define GOODIX_SCREENSTATE    0x17

int brl_receive_cmd(struct goodix_ts_core *cd, u8 rcv_cmd, u8 onoff) 
{
	struct goodix_ts_cmd cmd;
	ts_info("brl_receive_cmd");
	
	switch (rcv_cmd) {
		case GOODIX_GAME_CMD:
			...
			break;
		case GOODIX_CHARGING_CMD:
			cmd.cmd = GOODIX_CHARGING_CMD;
			cmd.len = 5;
			if (onoff) {
				cmd.data[0] = 0x01;
				cmd.data[1] = 0xB5;
		...
		struct ts_rawdata_info *info)
{
	...
	frame_buf = kmalloc(GOODIX_MAX_FRAMEDATA_LEN, GFP_KERNEL);
	if (!frame_buf) {
