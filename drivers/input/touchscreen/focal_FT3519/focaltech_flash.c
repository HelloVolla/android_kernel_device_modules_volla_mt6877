...
#define FTS_FW_NAME_PREX_WITH_REQUEST               "focal_firmware"
...
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
    u8 chipID,ctp_fw_version;
    fts_read_reg(FTS_REG_CHIP_ID, &chipID);
    fts_read_reg(0xA6, &ctp_fw_version);
    sprintf(current_tp_info.chip,"FT3519:0x%02x", ctp_fw_version);
    sprintf(current_tp_info.id,"0x%04x",chipID);
    strcpy(current_tp_info.vendor,"Focaltech");
    sprintf(current_tp_info.more,"%d*%d",1080,2436);
#endif
