...
static int mtk_dp_bind(struct device *dev, struct device *master, void *data)
{
	...
	if (ret) {
		DPTXERR("Failed to initialize connector: %d\n",
			ret);
		return ret;
	}
...
static int mtk_drm_dp_probe(struct platform_device *pdev)
{
	...
	if (irq_num < 0) {
		DPTXERR("failed to request dp irq resource\n");
		return -EPROBE_DEFER;
	}
	#endif
	...
	if (comp_id < 0) {
		DPTXERR("Failed to identify by alias: %d\n", comp_id);
		ret = comp_id;
		goto error;
	}
	ret = mtk_ddp_comp_init(dev, dev->of_node, &mtk_dp->ddp_comp, comp_id,
			NULL);
	if (ret) {
		DPTXERR("Failed to initialize component: %d\n", ret);
		goto error;
	}
	...
	if (ret) {
		DPTXERR("failed to request mediatek dptx irq\n");
		return -EPROBE_DEFER;
	}
	#endif
