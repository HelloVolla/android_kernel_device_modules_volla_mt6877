...
/* MT6899 DLI_Relay setting*/
#define MT6899_DISPSYS_DLI_RELAY_1TNP 0x200
#define MT6899_DISPSYS_DLI_NUM 4
#define MT6899_DISPSYS_DLO_RELAY_1TNP 0x230
#define MT6899_DISPSYS_DLO_NUM 5
#define MT6899_DISPSYS1_DLI_RELAY_1TNP 0x260
#define MT6899_DISPSYS1_DLI_NUM 5
#define MT6899_DLI_RELAY_1TNP REG_FLD_MSB_LSB(31, 30)

/* MT6991 DLI_Relay setting*/
#define MT6991_DISPSYS_DLI_RELAY_1TNP 0x200
#define MT6991_DISPSYS_DLI_NUM 16
#define MT6991_DISPSYS_DLO_RELAY_1TNP 0x264
#define MT6991_DISPSYS_DLO_NUM 9
...
static unsigned int dual_comp_blender_map_mt6991(unsigned int comp_id)
{
	...
	case DDP_COMPONENT_OVL1_EXDMA9:
		ret = DDP_COMPONENT_OVL1_BLENDER9;
		break;
#else
	case DDP_COMPONENT_OVL1_EXDMA9:
	...
	if (priv->data->mmsys_id == MMSYS_MT6991) {
		...
		if (crtc_id == 0)
			...
			if(comp->id < DDP_COMPONENT_OVL1_EXDMA0)
				...
				next_blender = DDP_COMPONENT_OVL1_BLENDER1 + plane_index;
#endif
		else if (crtc_id == 1)
			next_blender = dual_comp_blender_map_mt6991(comp->id);
		else if (crtc_id == 2)
...
void mtk_drm_crtc_mini_dump(struct drm_crtc *crtc)
{
	...
	case MMSYS_MT6991:
			DDPDUMP("== DISP pipe-0 OVLSYS_CONFIG REGS:0x%pa ==\n",
						&mtk_crtc->ovlsys0_regs_pa);
			ovlsys_config_dump_reg_mt6991(mtk_crtc->ovlsys0_regs);
			if (mtk_crtc->ovlsys1_regs) {
				DDPDUMP("== DISP pipe-1 OVLSYS_CONFIG REGS:0x%pa ==\n",
					&mtk_crtc->ovlsys1_regs_pa);
				ovlsys_config_dump_reg_mt6991(mtk_crtc->ovlsys1_regs);
			}

			DDPDUMP("== DISP pipe-0 MMSYS_CONFIG REGS:0x%pa ==\n",
						&mtk_crtc->config_regs_pa);
			mmsys_config_dump_reg_mt6991(mtk_crtc->config_regs);
			if (mtk_crtc->side_config_regs) {
				DDPDUMP("== DISP pipe-1 MMSYS_CONFIG REGS:0x%pa ==\n",
					&mtk_crtc->side_config_regs_pa);
...
void mtk_drm_crtc_dump(struct drm_crtc *crtc)
{
	...
	case MMSYS_MT6991:
		...
		mutex_dump_reg_mt6991(mtk_crtc->mutex[0]);
	...
	if (!crtc->state)
		...
		if ((state->lye_state.mml_dl_lye && priv->data->mmsys_id == MMSYS_MT6989) ||
			(state->lye_state.mml_dl_lye && priv->data->mmsys_id == MMSYS_MT6899) ||
...
void mtk_drm_crtc_mini_analysis(struct drm_crtc *crtc)
{
	...
	case MMSYS_MT6991:
			...
			DDPDUMP("== DUMP DISP pipe-0 ANALYSIS:0x%pa ==\n",
				&mtk_crtc->config_regs_pa);
			mmsys_config_dump_analysis_mt6991(mtk_crtc->config_regs, 0);
			if (mtk_crtc->side_config_regs) {
				DDPDUMP("== DUMP DISP pipe-1 ANALYSIS:0x%pa ==\n",
					&mtk_crtc->side_config_regs_pa);
...
void mtk_drm_crtc_analysis(struct drm_crtc *crtc)
{
	...
	case MMSYS_MT6768:
		...
		break;
	case MMSYS_MT6761:
		mmsys_config_dump_analysis_mt6761(mtk_crtc->config_regs);
		mutex_dump_analysis_mt6761(mtk_crtc->mutex[0]);
		break;
	case MMSYS_MT6765:
	...
	unsigned int panel_ext_param, unsigned int cfg_flag, unsigned int lock)
{
	...
	if (mtk_crtc_with_sub_path(crtc, mtk_crtc->ddp_mode))
		...
		mtk_crtc_wait_frame_done(mtk_crtc, cmdq_handle,
			DDP_FIRST_PATH, is_bdg_supported() ? 1: 0);
		...
		unsigned int cmd, void *params, bool need_lock)
{
	...
	mtk_drm_idlemgr_kick(__func__, crtc, 0);
	mtk_vidle_user_power_keep(DISP_VIDLE_USER_CRTC);	/* separate power keep after kick idle */
...
static void mtk_crtc_update_ovl_hrt_usage(struct drm_crtc *crtc)
{
	...
	drm_for_each_plane_mask(plane, crtc->dev, plane_mask) {
		...
		if (mtk_crtc && mtk_crtc->path_data && (mtk_crtc->path_data->is_exdma_dual_layer ||
		    mtk_crtc->is_dual_pipe) && priv) {
	...
	if (mtk_crtc && mtk_drm_dal_enable()) {
		...
		if (priv && priv->data->mmsys_id == MMSYS_MT6991) {
			...
			if (mtk_crtc->is_dual_pipe)
				mtk_crtc->usage_ovl_fmt[DISP_PMQOS_OVL1_EXDMA8] = 2;
		}
				      ...
				      struct cmdq_pkt *cmdq_handle)
{
	...
	if (lye_state->rpo_lye ||
		...
		for (pipe = 0; pipe < 2; ++pipe) {
			...
			if (!mtk_crtc->is_dual_pipe && addon_module[0]) {
				path_data = mtk_addon_module_get_path(addon_module[0]->module);
				reg_data = mtk_crtc->mmsys_reg_data;
				prev_comp_id = path_data->path[path_data->path_len - 1];
				if (reg_data && reg_data->dispsys_map &&
	...
	if (lye_state->mml_dl_lye) {
		...
		target_comp = addon_config.config_type.tgt_comp;
		if (!mtk_crtc->is_mml_dl_submit) {
			DDPMSG("comp:%d,L%d off\n", target_comp, addon_config.config_type.tgt_layer);
			mtk_ddp_comp_layer_off(priv->ddp_comp[target_comp],
...
static unsigned int dual_comp_map_mt6991(unsigned int comp_id)
{
	...
	case DDP_COMPONENT_OVL_EXDMA2:
		ret = DDP_COMPONENT_OVL1_EXDMA2;
		break;
	case DDP_COMPONENT_OVL_EXDMA3:
		ret = DDP_COMPONENT_OVL1_EXDMA3;
		break;
	case DDP_COMPONENT_OVL_EXDMA4:
		ret = DDP_COMPONENT_OVL1_EXDMA4;
		break;
	case DDP_COMPONENT_OVL_EXDMA5:
		ret = DDP_COMPONENT_OVL1_EXDMA5;
		break;
#if IS_ENABLED(CONFIG_MTK_DISPLAY_DUAL_PIPE_DUAL_PORT_SUPPORT)
	case DDP_COMPONENT_OVL_EXDMA6:
		ret = DDP_COMPONENT_OVL1_EXDMA6;
		break;
	case DDP_COMPONENT_OVL_EXDMA7:
		ret = DDP_COMPONENT_OVL1_EXDMA7;
		break;
	case DDP_COMPONENT_OVL_EXDMA8:
		ret = DDP_COMPONENT_OVL1_EXDMA8;
		break;
	case DDP_COMPONENT_OVL_EXDMA9:
		ret = DDP_COMPONENT_OVL1_EXDMA9;
		break;
	...
	for (i = mtk_crtc->layer_nr - 1; i >= 0; i--) {
		...
		if (plane_state->base.visible &&
			...
			if(priv->data->mmsys_id == MMSYS_MT6991
				...
				if (mtk_crtc->is_dual_pipe) {
					mtk_ddp_comp_layer_off(priv->ddp_comp[DDP_COMPONENT_OVL1_EXDMA2],
						comp_state->lye_id,comp_state->ext_lye_id,cmdq_handle);
				}
			}
			...
			for_each_comp_in_cur_crtc_path(
				...
				if (comp->id != comp_state->comp_id ||
				    (mtk_ddp_comp_get_type(comp->id) != MTK_DISP_OVL &&
				    mtk_ddp_comp_get_type(comp->id) != MTK_OVL_EXDMA))
					continue;
				      ...
				      struct cmdq_pkt *cmdq_handle)
{
	...
	if (mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_MAX_CHANNEL_HRT)) {
		...
		mtk_disp_get_channel_hrt_bw(mtk_crtc, channel_hrt,
				ARRAY_SIZE(channel_hrt));
		for (i = 0 ; i < ARRAY_SIZE(channel_hrt); i++) {
			ret = mtk_disp_set_per_channel_hrt_bw(mtk_crtc, channel_hrt[i],
						i, false, __func__);
			cmdq_pkt_write(cmdq_handle, mtk_crtc->gce_obj.base,
...
#if IS_ENABLED(CONFIG_MTK_LCM_DUAL_PORT_SUPPORT) || IS_ENABLED(CONFIG_MTK_DISPLAY_DUAL_PIPE_DUAL_PORT_SUPPORT)
	if (lyeblob_ids)
		frame_num = HRT_GET_SUM_LAYER_FLAG(lyeblob_ids->hrt_num);
#endif
		...
		unsigned int ddp_mode)
{
	...
	struct mtk_ddp_comp *comp;
	static u32 pre_rpo_lye;
	unsigned int cur_hrt_bw, cur_max_chan_bw, cur_chan_hrt_bw;
	...
	if (priv->data->update_channel_hrt_write) {
		for (i = 0; i < BW_CHANNEL_NR; i++) {
			...
			if (cur_chan_hrt_bw != NO_PENDING_HRT &&
				...
				DDPINFO("CRTC%u channel[%d]:%u, release HRT to last_channel_write_hrt_req:%u\n",
		...
		struct drm_atomic_state *atomic_state)
{
	...
	drm_for_each_plane_mask(plane, crtc->dev, plane_mask) {
		...
		dma_addr_t peak_slot = mtk_get_gce_backup_slot_pa(mtk_crtc,
			DISP_SLOT_LAYER_PEAK_RATIO(index));
		dma_addr_t pre_avg_slot = 0x0;
		dma_addr_t pre_peak_slot = 0x0;
		int lye_offset = 0x8, slot_offset = 0x4;
		...
		if ((display_compress_ratio_table[i].key_value) &&
			...
			if (!is_force_high_step &&
				...
				*(display_compress_ratio_table[i].peak_ratio) == 0) &&
				priv->data->mmsys_id == MMSYS_MT6991) {
				DDPINFO("%s ratio is 0, i:%d,frame_idx:%d,key value:%llu avg%d peak%d\n",
...
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct cmdq_pkt *bwm_handle;
	int index, ret = 0, lye_idx;
	dma_addr_t avg_slot = 0x0, ext_avg_slot = 0x0;
	dma_addr_t peak_slot = 0x0, ext_peak_slot = 0x0;
	int crtc_id = drm_crtc_index(&mtk_crtc->base);
	struct mtk_ddp_comp *comp = NULL;
	int type;
	int i, j;
		...
		if (type == MTK_OVL_EXDMA &&
			...
			for(index = 0; index < MT6991_MAX_EXDMA_NUM; index++) {
				...
				peak_slot = mtk_get_gce_backup_slot_pa(mtk_crtc,
						DISP_SLOT_LAYER_PRE_PEAK_RATIO(index));

				cmdq_pkt_mem_move(bwm_handle, mtk_crtc->gce_obj.base,
					comp->regs_pa + DISP_REG_OVL_LX_BURST_ACC(0)
					+ index * 0x10000, avg_slot, CMDQ_THR_SPR_IDX1);
				cmdq_pkt_mem_move(bwm_handle, mtk_crtc->gce_obj.base,
					comp->regs_pa + DISP_REG_OVL_LX_BURST_ACC_WIN_MAX(0)
					+ index * 0x10000, peak_slot, CMDQ_THR_SPR_IDX1);
...
static ktime_t mtk_check_preset_fence_timestamp(struct drm_crtc *crtc)
{
	...
	if (id == 0) {
		...
		if (div_u64(wait_time - start_time, 1000000) > (1000 / te_freq / 2)) {
	...
	struct cmdq_pkt *handle)
{
	...
	if (panel_ext && panel_ext->dsc_params.enable) {
		...
		case MMSYS_MT6853:
			...
			break;
		case MMSYS_MT6877:
			mtk_ddp_remove_dsc_prim_MT6877(mtk_crtc, handle);
			break;
		case MMSYS_MT6781:
	...
	struct cmdq_pkt *_handle)
{
	...
	if (comp && comp->id == DDP_COMPONENT_DSI1)
		...
		case MMSYS_MT6853:
			...
			break;
		case MMSYS_MT6877:
			mtk_ddp_insert_dsc_prim_MT6877(mtk_crtc, handle);
			break;
		case MMSYS_MT6781:
...
void mtk_crtc_connect_default_path(struct mtk_drm_crtc *mtk_crtc)
{
	...
	if (drm_crtc_index(crtc) == 0 && panel_ext &&
...
#else
			comp = mtk_crtc_get_comp_with_index(mtk_crtc, plane_state);
...
static int __mtk_check_trigger(struct mtk_drm_crtc *mtk_crtc)
{
	...
	if (next_tgt_dur > CT_PRETE_DUR) {
		DDP_MUTEX_UNLOCK(&mtk_crtc->lock, __func__, next_tgt_dur);
		...
		DDP_MUTEX_LOCK_CONDITION(&mtk_crtc->lock, __func__, next_tgt_dur, false);
...
static void _mtk_crtc_1tnp_setting(struct mtk_drm_crtc *mtk_crtc)
{
	...
	if (priv->data->mmsys_id == MMSYS_MT6989) {
		...
		SET_VAL_MASK(value, mask,
			mtk_crtc->dli_relay_1tnp, MT6991_DLI_RELAY_1TNP);

		base_addr = mtk_crtc->config_regs + MT6991_DISPSYS_DLI_RELAY_1TNP;
		for (i = 0; i < MT6991_DISPSYS_DLI_NUM; i++) {
			setting_addr = base_addr + (i * 0x4);
			val = readl(setting_addr);
			val = (val & ~mask) | (value & mask);
			writel(val, setting_addr);
		...
		base_addr = mtk_crtc->config_regs + MT6991_DISPSYS_DLO_RELAY_1TNP;
		for (i = 0; i < MT6991_DISPSYS_DLO_NUM; i++) {
			setting_addr = base_addr + (i * 0x4);
			val = readl(setting_addr);
			val = (val & ~mask) | (value & mask);
			writel(val, setting_addr);
		...
		base_addr = mtk_crtc->side_config_regs + MT6991_DISPSYS1_DLI_RELAY_1TNP;
		for (i = 0; i < MT6991_DISPSYS1_DLI_NUM; i++) {
			setting_addr = base_addr + (i * 0x4);
			val = readl(setting_addr);
			val = (val & ~mask) | (value & mask);
			writel(val, setting_addr);
		...
		base_addr = mtk_crtc->side_config_regs + MT6899_DISPSYS1_DLI_RELAY_1TNP;
		for (i = 0; i < MT6899_DISPSYS1_DLI_NUM; i++) {
			setting_addr = base_addr + (i * 0x4);
			val = readl(setting_addr);
			val = (val & ~mask) | (value & mask);
			writel(val, setting_addr);
...
void mtk_crtc_config_default_path(struct mtk_drm_crtc *mtk_crtc)
{
	...
	if (priv->data->mmsys_id == MMSYS_MT6983 ||
		...
		/*Set EVENT_GCED_EN EVENT_GCEM_EN <OVLSYS>*/
		writel(0x3, mtk_crtc->ovlsys0_regs +
				DISP_REG_CONFIG_OVLSYS_GCE_EVENT_SEL_MT6991);
		       ...
		       struct cmdq_pkt *cmdq_handle)
{
	...
	if (crtc && crtc->dev) {
		crtc_idx = drm_crtc_index(crtc);
		priv = crtc->dev->dev_private;
		if (priv && crtc_idx < MAX_CRTC)
			only_output = (priv->usage[crtc_idx] == DISP_OPENING);
	} else {
		DDPPR_ERR("%s: crtc is null\n", __func__);
		return;
	}
	...
	if (priv && priv->data && priv->data->mmsys_id == MMSYS_MT6991 && crtc_idx == 0) {
		...
		if (mtk_crtc->is_dual_pipe)
			mtk_ddp_comp_stop(priv->ddp_comp[DDP_COMPONENT_OVL1_EXDMA2],cmdq_handle);
	}
...
skip:
	...
	if (mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_MMQOS_SUPPORT)) {
		if (!(drm_crtc_index(crtc) == 0 && is_bdg_supported() &&
			priv->data->mmsys_id == MMSYS_MT6768))
	...
	const char *func, int line)
{
	...
	if (get) {
		...
		ret = wait_event_interruptible(priv->kernel_pm.wq,
				atomic_read(&priv->kernel_pm.status) != KERNEL_PM_SUSPEND);
		if (unlikely(ret != 0))
...
void mtk_crtc_first_enable_ddp_config(struct mtk_drm_crtc *mtk_crtc)
{
	...
	if (priv->data->mmsys_id == MMSYS_MT6985 ||
		...
		/*Set BYPASS_MUX_SHADOW*/
		writel(0x1, mtk_crtc->ovlsys0_regs +
				DISP_REG_CONFIG_OVLSYS_BYPASS_MUX_SHADOW_MT6991);
		writel(0x1fe0000, mtk_crtc->ovlsys0_regs +
	...
	if ((priv->data->mmsys_id == MMSYS_MT6991)
		...
		mtk_ddp_comp_first_cfg(priv->ddp_comp[DDP_COMPONENT_OVL_EXDMA2],
			&cfg, cmdq_handle);
			if (mtk_crtc->is_dual_pipe)
				mtk_ddp_comp_first_cfg(priv->ddp_comp[DDP_COMPONENT_OVL1_EXDMA2],
					&cfg, cmdq_handle);
		if (!mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_USE_PQ)) {
			mtk_crtc->pq_data->opt_bypass_pq = true;
			...
			if (mtk_crtc->is_dual_pipe)
				mtk_ddp_comp_bypass(priv->ddp_comp[DDP_COMPONENT_OVL1_EXDMA2],
					1, PQ_FEATURE_KRN_OPT_USE_PQ, cmdq_handle);
		} else {
			if (mtk_crtc->pq_data->opt_bypass_pq) {
				mtk_ddp_comp_bypass(priv->ddp_comp[DDP_COMPONENT_OVL_EXDMA2],
		...
		if (mtk_crtc->is_dual_pipe) {
			...
			mtk_ddp_comp_io_cmd(priv->ddp_comp[DDP_COMPONENT_OVL1_EXDMA2],
				cmdq_handle,
				MTK_IO_CMD_RDMA_GOLDEN_SETTING, &cfg);
			cfg.source_bpc = mtk_ddp_comp_io_cmd(priv->ddp_comp[DDP_COMPONENT_OVL1_EXDMA2],
				cmdq_handle, OVL_GET_SOURCE_BPC, NULL);
			/* update SRT BW */
			mtk_ddp_comp_io_cmd(priv->ddp_comp[DDP_COMPONENT_OVL1_EXDMA2],
				cmdq_handle, PMQOS_UPDATE_BW, NULL);
...
void mtk_drm_crtc_first_enable(struct drm_crtc *crtc)
{
	...
	if (mtk_drm_helper_get_opt(priv->helper_opt,
		...
		CRTC_MMP_MARK(drm_crtc_index(crtc), leave_vidle, 0xb007, 0x0);
						...
						bool need_sync_mml)
{
	...
	if (!mtk_crtc_is_frame_trigger_mode(crtc) &&
		...
		mtk_crtc_wait_frame_done(mtk_crtc, cmdq_handle, DDP_FIRST_PATH, clear_event);
		...
		enum MSYNC_RECORD_TYPE type, u64 time)
{
	...
	case FRAME_TIME:
		...
		DDPDBG("[Msync] min fps:%d, fps:%d, time_diff:%llu\n",
			MSYNC_MIN_FPS, fps, time_diff);
		if (1000ULL * 1000 * 1000 * 100 / MSYNC_MIN_FPS < time_diff) {
...
int mtk_drm_get_atomic_fps(void)
{
	...
	if (count == 0) {
		...
		if ((tval.tv_sec > sec) && (tval.tv_nsec/1000 < usec)) {
			...
			usec1 = 1000000 + DO_COMMON_DIV(tval.tv_nsec, 1000) - usec;
		}
...
#endif
	if (mtk_crtc->is_mml_dl)
		need_sync_mml = mtk_crtc->is_mml_dl_submit;
				      ...
				      struct drm_atomic_state *atomic_state)
{
	...
	if (mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_HBM)) {
		bool hbm_en = (bool)mtk_crtc_state->prop_val[CRTC_PROP_HBM_ENABLE];

		// When HBM enable via layer, use fingerprint HBM
...
//			mtk_atomic_hbm_bypass_pq(crtc, cmdq_handle, hbm_en);
	...
	if (!only_output) {
		...
		if ((priv->data->mmsys_id == MMSYS_MT6991)
			...
			if (mtk_crtc->is_dual_pipe) {
				if (crtc->state->color_mgmt_changed)
					mtk_ddp_gamma_set(priv->ddp_comp[DDP_COMPONENT_OVL1_EXDMA2],
					crtc->state, cmdq_handle);
				mtk_ddp_comp_io_cmd(priv->ddp_comp[DDP_COMPONENT_OVL1_EXDMA2],
					cmdq_handle, COMP_ODDMR_CFG, &mtk_crtc->sec_on);
				mtk_ddp_comp_io_cmd(priv->ddp_comp[DDP_COMPONENT_OVL1_EXDMA2],
	...
	if (mtk_crtc->is_mml || (mtk_crtc->is_mml_dl && mtk_crtc->is_mml_dl_submit))
				    ...
				    int buf_len, int event_id)
{
	...
	case EVENT_OVLSYS_DISP_OVL0_SOF:
		len = snprintf(buf, buf_len, "disp_ovlsys_ovl_exdma3_sof%d",
					drm_crtc_index(&mtk_crtc->base));
		break;
	default:
...
#define LEN 50
	...
	if (snprintf(name, LEN, "mtk_drm_cwb") < 0) {
		DDPPR_ERR("%s:%d snprintf error\n", __func__, __LINE__);
		return -EINVAL;
	}
	mtk_crtc->cwb_task =
...
void mtk_crtc_gce_event_config(struct drm_crtc *crtc)
{
	...
	if (priv->data->gce_event_config)
		priv->data->gce_event_config(crtc->dev);
}
