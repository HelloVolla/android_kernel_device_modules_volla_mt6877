...
int bypass_skip_trans;
module_param(bypass_skip_trans, int, 0644);
			  ...
			  int disp_index, int ovl_overlap_limit_w)
{
	...
	DDPINFO("final max_for_compare=%d\n", max_overlap);
	return max_overlap;
}
			       ...
			       int layer_map_idx)
{
	...
	if (disp_idx == 0) {
		if (HRT_GET_FIRST_SET_BIT(ovl_mapping_tb) >= layer_map_idx && priv->data->mmsys_id != MMSYS_MT6771)
		...
		if (mtk_drm_helper_get_opt(priv->helper_opt,
			...
			return DDP_COMPONENT_OVL4_2L;
		else if (priv->data->mmsys_id == MMSYS_MT6877)
			return DDP_COMPONENT_OVL1_2L;
		else
			return DDP_COMPONENT_OVL2_2L;
	} else if (disp_idx == 3) {
	...
	struct drm_device *drm_dev, int fun_lye)
{
	...
	if (disp_idx == 0) {
		...
		if (get_layering_opt(LYE_OPT_SPDA_OVL_SWITCH)) {
			...
			if (mtk_crtc->is_dual_pipe)
				exdma_comp = DDP_COMPONENT_OVL_EXDMA9;
				  ...
				  struct drm_device *drm_dev, struct mtk_lye_ddp_state *lye_state)
{
	...
	for (i = 0; i < disp_info->layer_num[idx]; i++) {
		...
		if (mtk_has_layer_cap(layer_info, MTK_MML_DISP_MDP_LAYER)) {
	...
	if (disp_idx == 0) {
		HRT_SET_NO_COMPRESS_FLAG(disp_info->hrt_num,
				no_compress_layer_num);
		HRT_SET_SUM_LAYER_FLAG(disp_info->hrt_num, sum_layer_num);
		DDPINFO("%s disp_info->hrt_num=0x%x,no_comp_layer_num=%d,sum_layer_num:%d\n",
			__func__, disp_info->hrt_num,
			no_compress_layer_num, sum_layer_num);
	}
	...
	const unsigned int hrt_idx)
{
				...
				if (!output_comp) {
					...
					if (output_comp && (mtk_ddp_comp_get_type(output_comp->id) == MTK_DSI)) {
						...
						mtk_ddp_comp_io_cmd(output_comp, NULL,
							DSI_GET_LINE_TIME_NS_BY_MODE, &ns);
					}
	...
	if (priv->data->skip_trans && !bypass_skip_trans && !priv->is_tablet) {
		mode = mtk_drm_crtc_avail_disp_mode(crtc, disp_info->disp_mode_idx[0]);
	...
	for (i = 0; i < disp_info->layer_num[disp_idx]; i++) {
		...
		if ((MTK_MML_DISP_DECOUPLE_LAYER & c->layer_caps) &&
			...
			if (!bypass_skip_trans && (real_te_duration > duration)) {
				DDPMSG("skip trans %d->%d\n", real_te_duration, duration);
				continue;
			}
			      ...
			      unsigned long arg)
{
	...
	for (i = 0; i < LYE_CRTC; i++) {
		...
		data.mml_cfg[i] = compat_ptr(data32.mml_cfg[i]);
	}
