...
/* drv modify fod status check start */
typedef int (*pfunc)(bool);
pfunc pts=NULL;
void set_fod_status_callback(void *f)
{
	pr_info("%s\n",__func__);
	if (IS_ERR_OR_NULL(pts)){
		pr_info("%s...\n",__func__);
		pts = f;
	}
}
EXPORT_SYMBOL(set_fod_status_callback);
int lcm_panel_set_fod_status(bool en)
{
	int ret;
	pr_info("%s\n",__func__);
	if (IS_ERR_OR_NULL(pts))
...
/* drv modify fod status check end */
...
lcm_dcs_write_seq_static(ctx,0x05,0x68); 
	...
	if (g_ctx->hbm_stat) {
		/* config backlight to max normal before HBM switch */
...
static int lcm_prepare(struct drm_panel *panel)
{
	...
	ctx->prepared = true;
	lcm_panel_set_fod_status(false);		/* drv modify fod status check  */
...
//drv modify fps 20241108-start
#define PCLK_IN_KHZ_60HZ \
    ((HAC+MODE_0_HFP+HSA+HBP)*(VAC+MODE_0_VFP+VSA+VBP)*(60)/1000)
#define PCLK_IN_KHZ_90HZ \
    ((HAC+MODE_1_HFP+HSA+HBP)*(VAC+MODE_1_VFP+VSA+VBP)*(90)/1000)
	
#define PCLK_IN_KHZ_120HZ \
    ((HAC+MODE_2_HFP+HSA+HBP)*(VAC+MODE_2_VFP+VSA+VBP)*(120)/1000)
			      ...
			      dcs_write_gce cb, void *handle)
{
	...
	{
		ctx->hbm_stat = false;
		level_normal = Gamma_to_level[bl_level];
	...
	void *dsi, dcs_write_gce cb, void *handle)
{
	...
	lcm_panel_set_fod_status(true);		/* drv modify fod status check  */
	return 0;
}
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	dsi->mode_flags = MIPI_DSI_MODE_LPM | MIPI_DSI_MODE_NO_EOT_PACKET | MIPI_DSI_CLOCK_NON_CONTINUOUS;  //prize add by yinhanhan for hbm 20230311
